...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({N(eosio.msig), N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake), N(eosio.vpay), N(eosio.rpay), N(eosio.bpay), N(eosio.saving) });
        ...
        BOOST_REQUIRE(active_schedule.producers.size() == 51);
