...
namespace eosiosystem {
   ...
   void system_contract::claimrewards( const account_name& owner ) {
      ...
      if( usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > 0 ) {
         ...
         auto rest_pay           = to_producers - to_per_block_pay;
		 auto to_per_vote_pay = rest_pay / 3;
		 auto to_per_rank_pay = rest_pay - to_per_vote_pay;
         ...
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {N(eosio),N(active)},
                                                       { N(eosio), N(eosio.rpay), asset(to_per_rank_pay), "fund per-rank bucket" } );

		 INLINE_ACTION_SENDER(eosio::token, transfer)(N(eosio.token), { N(eosio),N(active) },
		                                               { N(eosio), N(eosio.vpay), asset(to_per_vote_pay), "fund per-vote bucket" });

		 _gstate.pervote_bucket += to_per_vote_pay;
		 _gstate.perrank_bucket += to_per_rank_pay;
	  ...
	  int64_t producer_per_rank_pay = 0;
	  ...
	  {
		  if (_gstate.total_producer_vote_weight > 0) {
			  // 70% of _gstate.perrank_bucket is shared equally between BPs
			  auto equal_share = int64_t((((double)_gstate.perrank_bucket) * 0.7) / 51.0);
		  ...
		  if (producer_per_rank_pay < min_pervote_daily_pay) {
			  producer_per_rank_pay = 0;
		  }
		  _gstate.perrank_bucket -= producer_per_rank_pay;
	  }
	  ...
	  if (producer_per_rank_pay > 0) {
		  INLINE_ACTION_SENDER(eosio::token, transfer)(N(eosio.token), { N(eosio.rpay),N(active) },
		                                               { N(eosio.rpay), owner, asset(producer_per_rank_pay), std::string("producer rank pay") });
	  }
   }
